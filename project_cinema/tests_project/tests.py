...
def test_update_movie(client, set_up):
    ...
    new_actors = [random_person().name] # tylko 1 aktor, powinno być kilku; do sprawdzenia później i poprawienia
    ...
    db_actors_names = [actor.name for actor in movie_obj.actors.all()]
    assert len(db_actors_names) == len(new_actors)
